...
from agimus_demo_09_glue_spreading.mpcTrajectoryUtils import PatternGenerator, SplineGenerator
import yaml
...
class TrajectoryMPC(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('plate_param', "/home/gepetto/ros2_ws/src/agimus-demos/agimus_demo_09_glue_spreading/config/plate.yaml")
        # Load plate configuration
        config_path = Path(self.get_parameter('plate_param').get_parameter_value().string_value)
        ...
        self.tf_object_listener = TransformListener(self.tf_object_buffer, self,spin_thread=True)
        self.last_tf = [None,None] # To store the last transforms
        self.tf = [None,None]  # To store the latest transforms

        # Publisher setup 
        self.traj_marker_publisher = self.create_publisher(Marker, 'trajectory_marker', 10)
    ...
    def wait_for_transform(self, parent_frame, child_frame):
        while not self.tf_object_buffer.can_transform(parent_frame, child_frame, rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=0.5)):
            self.get_logger().info(f"Waiting for TF between {parent_frame} and {child_frame}...")
            rclpy.spin_once(self, timeout_sec=0.1)
        self.get_logger().info("TF between base_link and object_frame is available.")
    
    def get_tf(self, parent_frame, child_frame):
        try:
            if not self.tf_object_buffer.can_transform(parent_frame, child_frame, rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=0.25)):
                self.get_logger().warn(f'TF entre {parent_frame} et {child_frame} indisponible.')
                return 
            ...
            return [trans.transform.translation, trans.transform.rotation] # Store translation and rotation
    ...
    def display_trajectory_rviz(self):
        ...
        marker = Marker()
        marker.header.frame_id = self.REF_FRAME  # ou "map", "base_link", selon ton robot
